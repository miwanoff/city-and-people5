...
if (!lmm_is_plugin_active('leaflet-maps-marker/leaflet-maps-marker.php') ) {
	...
	if ($action_standalone == NULL) {
		...
		(
			...
			$option_google_inactive = '<tr><td colspan="2"><strong>' . esc_attr__('Inactive (API key required)','lmm') . '</strong></td></tr><tr><td><input id="google" type="radio" name="geocoding-provider" value="google-geocoding" ' . checked($lmm_options['geocoding_provider'], 'google-geocoding', false) . 'disabled="disabled" /><label for="google">Google Geocoding</label></td><td>'.sprintf(__('For details about pricing and usage limits <a href="%1$s" target="_blank">please click here</a>','lmm'), 'http://mapsmarker.com/google-geocoding'). $geocoding_provider_api_key_warning_google . '</td></tr>';
			$option_google_active = '';
		} else {
			$option_google_active = '<tr><td><input id="google" type="radio" name="geocoding-provider" value="google-geocoding" disabled="disabled" ' . checked($lmm_options['geocoding_provider'], 'google-geocoding', false ) . '/><label for="google">Google Geocoding</label></td><td>'. sprintf(__('For details about pricing and usage limits <a href="%1$s" target="_blank">please click here</a>','lmm'), 'http://mapsmarker.com/google-geocoding') .'</td></tr>';
		...
		if ($action_standalone == 'export') {
			...
			**********************************/
			...
			if ( ($_POST['filter-markername'] == NULL) || ($_POST['filter-popuptext'] == NULL) ) {
					...
					//hotfixed due potential security issue $filter_option1_sql .= ' ' . esc_sql($_POST['filter-operator1']) . ' ';
					$filter_option1_sql .= ' ';
			}
			...
			if ( ($_POST['filter-exclude-markername'] == NULL) || ($_POST['filter-exclude-popuptext'] == NULL) ) {
					...
					//hotfixed due potential security issue $filter_option2_sql .= ' ' . esc_sql($_POST['filter-operator2']) . ' ';
					$filter_option2_sql .= ' ';
			}
